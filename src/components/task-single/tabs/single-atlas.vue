		...
		<transition name="fade">
			<div class="atals-show" v-show="ifAtals">
				<div class="atals-show-warpper" @click="hideAtals">
					<div class="atals-close" @click.stop="hideAtals">
...
<script>
	...
	import { getAtlasList } from 'api/single-atlas'
	...
	export default {
		...
		methods: {
			...
			getNewDownHref() {
				setTimeout(() => {
					this.downLoadHref = `${httpUrl}patrol/download_atlas?constructionid=${this.name}&ym=${this.cascaderData[0]}-${this.cascaderData[1]}`				
				}, 20)
			},
			getYear() {
			...
			search() {
				...
				this._getAtlasList(search)
			},
			_getAtlasList(search) {
				this.showLoading = true
				getAtlasList(search).then(res => {
					if(res.code === ERR_OK) {
